C
     ...
     & LCSP1,LCSP2,LCRC1,LCRC2,NGSITH,MXGSIP,INGSI)
C **********************************************************************
...
C
 ...
 1039 FORMAT(1X,'TYPE OF SORPTION SELECTED IS [GSI]')
C
      IF(IRCTOP.LE.1) THEN
        IRCTOP=1
        WRITE(*,1050)
 ...
 1052 FORMAT(1X,'REACTION COEFFICIENTS ASSIGNED CELL-BY-CELL')
      IF(IGETSC.EQ.0) THEN
        WRITE(IOUT,1060)
      ELSEIF(IGETSC.GT.0.AND.(ISOTHM.LE.3.OR.ISOTHM.EQ.7)) THEN
        WRITE(*,1061)
        CALL USTOP(' ')
      ELSEIF(IGETSC.GT.0.AND.
     $       (ISOTHM.EQ.4.OR.ISOTHM.EQ.5.OR.ISOTHM.EQ.6)) THEN
        WRITE(IOUT,1062)
      ENDIF
...
C
     ...
     &          J,I,K,JR,IR,KR,IRCTOP,NCOMP,IGETSC,INDEX, INGSI,
     &          NGSITH, MXGSIP, IERR, IGSIT, NGSIP
...
C--GSI ISOTHERM READ
      IF(ISOTHM.EQ.7) THEN
         DO K=1,NGSITH
            READ(INGSI,*, IOSTAT=IERR) NGSIP, IGSIT
            IF(IERR.NE.0) THEN
               IGSIT=1
               READ(INGSI,*) NGSIP
            ENDIF
            ...
            DO J=1,NGSIP
               IF(IGSIT.EQ.1) THEN
                  READ(INGSI,*) XGSI(K,J,1), XGSI(K,J,2)
                  XGSI(K,J,3)=0
                  XGSI(K,J,4)=0
               ELSEIF(IGSIT.EQ.2) THEN                  
                  READ(INGSI,*) XGSI(K,J,1), XGSI(K,J,2), XGSI(K,J,3)
                  XGSI(K,J,4)=0
               ELSEIF(IGSIT.EQ.3) THEN
                  READ(INGSI,*, IOSTAT=IERR) XGSI(K,J,1), XGSI(K,J,2)
     $                 , XGSI(K,J,3), XGSI(K,J,4)
                  IF(IERR.NE.0) THEN
                     XGSI(K,J,4)=1
                     READ(INGSI,*) XGSI(K,J,1), XGSI(K,J,2)
     $                 , XGSI(K,J,3)
...
C--OR IMMOBILE-LIQUID PHASE CONCENTRATION (DUAL-DOMAIN MODEL)
     ...
     &     SRCONC(1,1,1,INDEX),ISOTHM,IREACT,TR,XGSI, NGSITH, MXGSIP)
        ENDDO
      ENDIF
C
...
C--OF 1ST ORDER REACTION TERM IF AN EXPLICIT SOLUTION SCHEME IS USED
      ...
      IR=0
      JR=0
      IF(IREACT.ne.1.AND.(ISOTHM.LE.3.OR.ISOTHM.EQ.7)) GOTO 4000
C
      DO INDEX=1,NCOMP
        DO K=1,NLAY
          DO I=1,NROW
...
C
     ...
     & ISOTHM,IREACT,DTRANS,XGSI, NGSITH, MXGSIP)
C ********************************************************************
...
C
      IMPLICIT  NONE
      INTEGER   NCOL,NROW,NLAY,ICBUND,ISOTHM,IREACT,J,I,K, ii,
     $          IGSITH, NGSITH, MXGSIP
      REAL      PRSITY,COLD,RETA,RFMIN,RHOB,SP1,SP2,RC1,RC2,
     &          PRSITY2,FRAC,SRCONC,DTRANS,TINY,
     &          RETA2,TERM1,RC1TMP,RC2TMP, XGSI, GSIM, GSIV,
     $          GSICC, GSILK, GSILS, GSIEXP
     ...
     &          RETA2(NCOL,NROW,NLAY) ,XGSI(NGSITH,0:MXGSIP,4)
            ...
            DO I=1,NROW
               DO J=1,NCOL
                  ...
                  IF(COLD(J,I,K).LT.0) THEN
                     ...
                     SRCONC(J,I,K)=0.
                  ELSE
                     IGSITH=INT(SP1(J,I,K))
C--7.1               Piecewise linear interpolated isotherm
                     IF(INT(XGSI(IGSITH,0,2)).EQ.1) THEN
                        GSIV=-1
                        DO ii=1,INT(XGSI(IGSITH,0,1))-1
                           IF(XGSI(IGSITH,ii,1).EQ.COLD(J,I,K)) THEN 
                              GSIV=XGSI(IGSITH,ii,2)
                              GSIM=(XGSI(IGSITH,ii-1,2)-XGSI(IGSITH,ii+1,2))/
     $                             (XGSI(IGSITH,ii-1,1)-XGSI(IGSITH,ii+1,1))    
                           ELSEIF(XGSI(IGSITH,ii,1).LT.COLD(J,I,K).AND.XGSI(IGSITH,ii+1,1).GT.COLD(J,I,K)) THEN
                              GSIM=(XGSI(IGSITH,ii,2)-XGSI(IGSITH,ii+1,2))
     $                             /(XGSI(IGSITH,ii,1)-XGSI(IGSITH,ii+1,1))
                              GSIV=XGSI(IGSITH,ii,2)+GSIM*(COLD(J,I,K)-XGSI(IGSITH,ii,1))
                           ENDIF


                        ENDDO 
                        IF(GSIV.EQ.-1) THEN
                           WRITE(*,*) "Concentracion ", COLD(J,I,K),
     $                          " out of bounds at isotherm ", IGSITH
                        ENDIF
C--7.2               Spline interpolated isotherm
                     ELSEIF(INT(XGSI(IGSITH,0,2)).EQ.2) THEN

C--7.3               Multistep Langmuir isotherm
                     ELSEIF(INT(XGSI(IGSITH,0,2)).EQ.3) THEN
                        GSIV=0.0
                        GSIM=0.0
                        DO ii=1,INT(XGSI(IGSITH,0,1))
                           IF(COLD(J,I,K).GE.XGSI(IGSITH,ii,3)) THEN
                              GSICC=COLD(J,I,K)-XGSI(IGSITH,ii,3)
                              GSILS=XGSI(IGSITH,ii,2)
                              GSILK=XGSI(IGSITH,ii,1)
                              GSIEXP=XGSI(IGSITH,ii,4)
                              GSIV=GSIV+GSILS*GSILK*GSICC**GSIEXP/(1.0+GSILK*GSICC**GSIEXP)
                              GSIM=GSIM+GSILS*GSILK*GSIEXP*GSICC**(GSIEXP-1)/((1.0+GSILK*GSICC**GSIEXP)**2)
                           ENDIF
                        ENDDO
                     ENDIF                     
C--               Calc RETA and SRCONC:
                     SRCONC(J,I,K)=GSIV
                     RETA(J,I,K)=1. + RHOB(J,I,K)/PRSITY(J,I,K) * GSIM
                  ENDIF
                  RFMIN=MIN(RFMIN,RETA(J,I,K))
               ENDDO
            ENDDO
...
C         DO ii=1,NCOL
C            WRITE(99,*) COLD(ii,1,1), ", ", RETA(ii,1,1), ", ", SRCONC(ii,1,1) 
C         ENDDO
         WRITE(99,*) ( COLD(ii,1,1), ii=1,NCOL)
         WRITE(99,*) ( RETA(ii,1,1), ii=1,NCOL)
         WRITE(99,*) ( SRCONC(ii,1,1), ii=1,NCOL)
...
C--SORBED PHASE FOR EQUILIBRIUM-CONTROLLED ISOTHERMS
     ...
     &           *DELR(J)*DELC(I)*DH(N)*SP1(N,ICOMP)
             ELSEIF(ISOTHM.EQ.2.OR.ISOTHM.EQ.3.OR.ISOTHM.EQ.7) THEN
...
C
C--SORBED PHASE FOR EQUILIBRIUM-CONTROLLED ISOTHERMS
              IF(ISOTHM.EQ.1.OR.ISOTHM.EQ.2.OR.ISOTHM.EQ.3.
     $             OR.ISOTHM.EQ.7) THEN
...
C
     ...
     & RC2,PRSITY2,RETA2,FRAC,CNEW,RETA,RFMIN,RMASIO,XGSI,NGSITH,MXGSIP)
C **********************************************************************
...
C
     ...
     &          CMML,CMMS,CIML,CIMS,VOLUME,DCRCT2,XGSI
...
C
     ...
     &   SRCONC(1,1,1,ICOMP),ISOTHM,IREACT,DTRANS,XGSI, NGSITH, MXGSIP)
      ENDIF
C
...
C
     ...
     & SRCONC,ISOTHM,IREACT,DTRANS,XGSI, NGSITH, MXGSIP)
C ********************************************************************
...
C
     ...
     &          RC1,RC2,PRSITY2,RETA2,FRAC,XGSI
     ...
     &          RC1(NCOL,NROW,NLAY,NCOMP),RC2(NCOL,NROW,NLAY,NCOMP),
     &          PRSITY2(NCOL,NROW,NLAY),FRAC(NCOL,NROW,NLAY),
     &          RETA2(NCOL,NROW,NLAY,NCOMP),XGSI(NGSITH,0:MXGSIP,4)
C
      IF(ISOTHM.EQ.2.OR.ISOTHM.EQ.3.OR.ISOTHM.EQ.7) THEN
        CALL SRCT5R(NCOL,NROW,NLAY,ICBUND(1,1,1,ICOMP),PRSITY,
