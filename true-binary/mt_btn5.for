          ...
          IF(LINE(ITYP1:ITYP2).EQ.NameTRNOP(I)) THEN
            IF(IU.EQ.0) THEN          
              ...
              elseif(NameTRNOP(i).EQ.'HSS') THEN
                IU=INHSS                                                                             
              elseif(NameTRNOP(i).EQ.'GSI') THEN
                IU=INGSI
              else
        ...
        DO K=1,NLAY
          DO I=1,NROW
            DO J=1,NCOL
             ...
             ELSEIF(ISOTHM.EQ.2.OR.ISOTHM.EQ.3.OR.ISOTHM.EQ.4.
     $               OR.ISOTHM.EQ.7) THEN			  
      ...
      DO K=1,NLAY
        DO I=1,NROW
          DO J=1,NCOL
            ...
            ELSEIF(ISOTHM.EQ.2.OR.ISOTHM.EQ.3.OR.ISOTHM.EQ.4.OR.
     $             ISOTHM.EQ.7) THEN
...
C--PRINT NONLINEAR RETARDATION FACTOR IF NEEDED
   40 IF(iUnitTRNOP(4).EQ.0) GOTO 50
      IF(ISOTHM.NE.2.AND.ISOTHM.NE.3.AND.ISOTHM.NE.7) GOTO 50
...
C
     ...
     &   WRITE(IOUT,1172) TMASIO(120,1,ICOMP),TMASIO(120,2,ICOMP)
      IF(iUnitTRNOP(4).GT.0.AND.(ISOTHM.EQ.5.OR.ISOTHM.EQ.6)) THEN
        WRITE(IOUT,1173)
