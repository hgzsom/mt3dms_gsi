...
C
     ...
     &          IOUT,ICNF,IUCN,IUCN2,IOBS,IMAS,ICBM,ISSGOUT,INGSI,
     ...
     &          faclength,factime,facmass, XGSI
     ...
     &          XGSI(:,:,:)
      ALLOCATABLE :: X,IX,cobsnam,fobsnam,HSSNAM, XGSI      
...
C-Open files using the Name File method as in MODFLOW-2000      
  ...
  104 FORMAT(1x,'Using NAME File: ',a)
     ...
     & INGCG,INTOB,INHSS,INFTL, INGSI,FPRT,MXTRNOP,iUnitTRNOP,NameTRNOP)
      CLOSE (iNameFile)      
C
...
C--ALLOCATE STORAGE SPACE FOR DATA ARRAYS
     ...
     & LCPRSITY2,LCRETA2,LCFRAC,LCSP1,LCSP2,LCRC1,LCRC2,NGSITH,MXGSIP,
     $ iUnitTRNOP(16))
...
C-- GSI: allocate XGSI
      ALLOCATE(XGSI(NGSITH,0:MXGSIP,4))
...
C--THE ENTIRE SIMULATION
     ...
     & iUnitTRNOP(16), XGSI, NGSITH, MXGSIP)
      IF(iUnitTRNOP(5).GT.0) 
...
C--UPDATE COEFFICIENTS THAT VARY WITH ITERATIONS
     ...
     &           X(LCFRAC),X(LCSR),ISOTHM,IREACT,DTRANS,XGSI, NGSITH, MXGSIP)
C
...
C
     ...
     &         X(LCCNEW),X(LCRETA),RFMIN,RMASIO,XGSI,NGSITH,MXGSIP)
C
